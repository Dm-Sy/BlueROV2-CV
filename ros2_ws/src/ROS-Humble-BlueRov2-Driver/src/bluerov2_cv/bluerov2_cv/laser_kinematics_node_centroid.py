...
class LaserKinematicsNodeCentroid(Node):
    def __init__(self):
        super().__init__('laser_kinematics_node_centroid')

        # --- KONFIGURACIJA KAMERE I LASERA ---
        self.B_METERS = 0.8  # Udaljenost između lasera
...
def main(args=None):
    ...
    node = LaserKinematicsNodeCentroid()
    try:
        rclpy.spin(node)
